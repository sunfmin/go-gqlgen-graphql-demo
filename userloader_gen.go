...
func (l *UserLoader) Load(key string) (*api.User, error) {
...
func (l *UserLoader) LoadThunk(key string) func() (*api.User, error) {
	l.mu.Lock()
...
func (l *UserLoader) LoadAll(keys []string) ([]*api.User, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *UserLoader) LoadAllThunk(keys []string) func() ([]*api.User, []error) {
	results := make([]func() (*api.User, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *UserLoader) Prime(key string, value *api.User) bool {
	l.mu.Lock()
	var found bool
...
func (l *UserLoader) unsafeSet(key string, value *api.User) {
	if l.cache == nil {
		l.cache = map[string]*api.User{}
	}
	l.cache[key] = value
}
